...
import { ContactsClient } from '../../ui/src/contacts-client.js';
import { ContactsStore } from '../../ui/src/contacts-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/contacts_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new ContactsStore(
  	new ContactsClient(player.appWs as any, 'contacts_test'),
  );
  return {
  	store,
  	player,
